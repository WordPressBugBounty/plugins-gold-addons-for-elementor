(function ($) {
    ...
    var GoldAddonsCountDownHandler = function ($scope, $) {

        var $countDownElement = $scope.find(".goldaddons-countdown"),
        ...
        if ($countDownElement.find('#countdown-' + id).hasClass('goldaddons-countdown-flip')) {
            settings.format.split('').forEach(function (unit) {
                var lowercased = unit.toLowerCase();

                layout += '<div class="goldaddons-countdown-block goldaddons-countdown-' + lowercased + '"><div class="pre_time-mid"> <div class="goldaddons-countdown-figure"><span class="top">{' + lowercased + 'nn}</span><span class="top-back"><span>{' + lowercased + 'nn}</span></span><span class="bottom">{' + lowercased + 'nn}</span><span class="bottom-back"><span>{' + lowercased + 'nn}</span></span></div><span class="goldaddons-countdown-label">{' + lowercased + 'l}</span></div><span class="countdown_separator">{sep}</span></div>';
            });
        }
        ...
        if (settings.reset) {
            $countDownElement.find('.goldaddons-countdown-init').countdown('option', 'until', new Date(until));
        }
        ...
        function animateFigure(periods, map) {
            settings.format.split('').forEach(function (unit) {
                ...
                if (periods[index] !== oldVal) {
                    ...
                    var $top = $('#countdown-' + id).find('.goldaddons-countdown-' + lowercased + ' .top'),
                        $back_top = $('#countdown-' + id).find('.goldaddons-countdown-' + lowercased + ' .top-back');
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/goldaddons-countdown-timer.default', GoldAddonsCountDownHandler);
    });
 })(jQuery);
